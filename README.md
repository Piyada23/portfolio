...
* #### initial 
   ...
   **remark:** คำสั่ง `magel.begin();` และ `magel.centric.begin(); ` ผู้ใช้งานต้องเลือกใช้คำสั่งใดคำสั่งหนึ่ง
 * #### info
 ...
 * #### sensor
   ...
   * `magel.sensor.report();` สั่งเพื่อใช้ในการนำ Key Sensor ที่ทำการ Add เข้าไปในชุดข้อมูล sensor และทำการส่ง report ขึ้น Magellan 
   **remark:** ชุดข้อมูลของ Sensor ที่ถูก add ไว้จะถูก Clear ทันทีเมื่อมีการ Report ค่าขึ่น Magellan
 * #### builtinSensor
   * `magel.builtinSensor.readTemperature(); `
   * `magel.builtinSensor.readHumidity(); `
 * #### report
   * `magel.report.send(Content type); `
      * `magel.report.send(JSON);`
      * `magel.report.send(String key, String value);`
      * `magel.report.send(Int UNIXtimestamp, String payload);`
   * `magel.begin();`
   * `magel.begin();`
   * `magel.begin();`
   * `magel.begin();`
